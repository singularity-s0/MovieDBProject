...
std::nullopt_t redirect_to_mycenter(
    ...
    bserv::db_result db_res = tx.exec("select tickets.ticket_id as ticket_id, users.username as username, screenings.showing_date as showing_date, screenings.time as time, screenings.room as room, seats.seat_name as seat_name, tickets.price as price, tickets.refunded as refunded from tickets, auth_user as users, screenings, seats where tickets.user_id=? and tickets.user_id=users.id and tickets.screening_id=screenings.screening_id and tickets.seat_id=seats.seat_id;", uid);
    lginfo << db_res.query();
